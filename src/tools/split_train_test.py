    ...
    for annotation_file in annotations:
        ...
        for i, row in txt_to_frame(annotation_file, headers=True).iterrows():
            ...
            if class_id in ids_to_keep:
                ...
                values.append(
                    [str(image_path), classes[class_id], w, h, x, y, x + w, y + h]
                )
...
def split_train_test(
    ...
    annotation_files = [
        annotation for annotation in Path(annotations_path).glob("[!classes]*.txt")
    ]
    ...
    logger.info(
        f"Original file count: {len(annotation_files)}. "
        f"Filtered file count: {len(filtered_annotations)}."
    )

    train_annotations, test_annotations = train_test_split(
        filtered_annotations, random_state=random_state, test_size=test_size
    )
    logger.info(
        f"Train file count: {len(train_annotations)}. "
        f"Test file count: {len(test_annotations)}"
    )

    train_frame = annotation_to_frame(
        images_path, train_annotations, classes, ids_to_keep
    )
    test_frame = annotation_to_frame(
        images_path, test_annotations, classes, ids_to_keep
    )
    ...
    if not dry_run:
        ...
        logger.info(
            f"Written to {train_file_path}, {test_file_path} and {class_output_path}"
        )
    else:
        logger.info(
            f"Would have written to {train_file_path}, {test_file_path} and {class_output_path}"
        )
